        ...
        it "raises exception" do
          FakeFS do
            ...
            Dir.chdir("/absolute/path/to") do
              ...
              }.should raise_error(SearchPathNotAbsoluteError, "Search path(s) [\"a/not/existing/relative/path\"] is not absolute.")
            end
          end
        end
      end
        ...
        context "given not existing search path" do
          context "as absolute path" do
            it "raises exception if path not exists" do
              ...
              }.should raise_error(SearchPathNotExistError, "Search path(s) [\"/not/existing/path\"] does not exists.")
            end
          end
          ...
          it "raises exception if path is not a directory" do
            FakeFS do
              ...
              }.should raise_error(SearchPathNotExistError, "Search path(s) [\"/existing/path/to/file.txt\"] does not exists.")
            end
          end
        end
      end
    ...
    describe "#find!" do
      context "empty search path given" do
        it "should raise exception" do
          lambda { SearchPath.new(nil).find!("searched_file.txt") }.should raise_error(FileNotFoundError,
                                                                                       "File 'searched_file.txt' not found in search paths [].")
          lambda { SearchPath.new([]).find!("searched_file.txt") }.should raise_error(FileNotFoundError)
        end
      end
      ...
      context "search path given" do
        context "file not exists in search path" do
          it "should raise exception" do
            FakeFS do
              search_path = SearchPath.new(["/not/existing/path"])
              lambda { search_path.find!("searched_file.txt") }.should raise_error(FileNotFoundError)
            end
          end
        end
